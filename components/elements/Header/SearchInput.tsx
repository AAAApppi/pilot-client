...
import { getCatalogByNameFx, searchCatalogFx } from '@/app/api/course'
import {
  NoOptionsMessage,
  NoOptionsSpinner,
...
const SearchInput = () => {
  ...
  const spinner = useStore(searchCatalogFx.pending)
  ...
  const searchPart = async (search: string) => {
    try {
      ...
      const data = await searchCatalogFx({
        url: '/catalog/search',
        search,
      })

      const names = data
        .map((item: ICourse) => item.coursename)
  ...
  const getPartAndRedirect = async (name: string) => {
    const part = await getCatalogByNameFx({
      url: '/catalog/name',
      name,
    })

    if (!part.id) {
      toast.warning('Курс не найден.')
      return
    }
